        ...
        return (
            <div className="portfolio_page">
                <div className="filter_header">
                    ...
                    <FilterButton filteredIn={this.state.filteredIn} filteredOut={this.state.filteredOut} text="Other" onClick={(urmom) => this.handleSelect(urmom)} selected={this.state.selected} />
                </div>
                <div className="project_container">
                    ...
                    <ProjectPreview display={this.state.selected.length < 1 ? null :
                    !this.state.selected.every(el => (this.state.filterCombos[1]).includes(el))? "hide_project" : null} 
                    title="Other" src="Portfolio.PNG" url="https://tylersportfolio.netlify.app/" stack="JavaScript"/>
                    <ProjectPreview display={this.state.selected.length < 1 ? null :
                    !this.state.selected.every(el => (this.state.filterCombos[2]).includes(el))? "hide_project" : null} 
                    title="Third" src="Portfolio.PNG" url="https://tylersportfolio.netlify.app/" stack="React JavaScript"/>
                </div>
            </div>
        )
    }
